...
(defun my/copilot-mode-if-not-daemon ()
  "Enable copilot-mode only when not running as daemon."
  (unless (daemonp)
    (copilot-mode 1)))

(use-package! copilot
  :hook ((prog-mode . my/copilot-mode-if-not-daemon)
         (git-commit-mode . my/copilot-mode-if-not-daemon)
         (with-editor-mode . my/copilot-mode-if-not-daemon))
...
(after! emacs-everywhere
  ...
  ;; Store emacs-everywhere messages in nextcloud
